    ...
    <FormProvider methods={methods} onSubmit={handleSubmit(onSubmit)}>
      <Card sx={{ p: 3 }}>
        ...
        <Grid container spacing={2}>
              ...
              <Grid item xs={12}>
                <Card variant="outlined" sx={{ p: 2, mb: 2 }}>
                  <Grid container spacing={2} alignItems="center">
                    ...
                    <Grid item xs={12} md={6}><RHFTextField name={`projects[${index}].domain`} label="Domain" fullWidth required /></Grid>
                    <Grid item xs={12} md={6}><RHFTextField name={`projects[${index}].projectTitle`} label="Project Title" fullWidth required /></Grid>
                    <Grid item xs={12} md={6}><RHFTextField name={`projects[${index}].dateOfStart`} label="Start Date" type="date" fullWidth InputLabelProps={{ shrink: true }} required /></Grid>
                    <Grid item xs={12} md={6}><RHFTextField name={`projects[${index}].dateOfEnd`} label="End Date" type="date" fullWidth InputLabelProps={{ shrink: true }} required /></Grid>
                    <Grid item xs={12} md={6}>
                      <RHFSelect name={`projects[${index}].location`} label="Location" fullWidth required>
                        {locationOptions.map((o) => <option key={o.value} value={o.value}>{o.label}</option>)}
                      </RHFSelect>
                    </Grid>
                    <Grid item xs={12}><RHFTextField name={`projects[${index}].teamInformation`} label="Team Information (USN & Name)" multiline fullWidth required /></Grid>
                    <Grid item xs={12}><RHFTextField name={`projects[${index}].projectDescription`} label="Project Description" multiline fullWidth required /></Grid>
                  </Grid>
                </Card>
              </Grid>
            </React.Fragment>
          ...
          <Grid item xs={12}>
            <Button variant="contained" color={isLight ? "primary" : "info"} onClick={() => append({ ...DEFAULT_EMPTY_PROJECT })} sx={{ mt: 2, display: "block", mx: "auto" }}>
              Add Project
            </Button>
          </Grid>
          <Grid item xs={12}>
            <Stack spacing={3} alignItems="flex-end" sx={{ mt: 3 }}>
              <Box display="flex" gap={1}>
                <LoadingButton variant="outlined" color={isLight ? "primary" : "info"} onClick={() => reset({ projects: [{ ...DEFAULT_EMPTY_PROJECT }] })}>Reset</LoadingButton>
