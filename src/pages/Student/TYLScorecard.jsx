...
const TYLScorecard = () => {
  ...
  const { data: cacheData, loading, error, invalidate } = useApiCache(
    userId ? `/tyl-scores/${userId}` : null
  );

  useEffect(() => {
    if (cacheData !== undefined) {
      if (cacheData?.status === 'success' && Array.isArray(cacheData.data)) {
        setTYLScores(cacheData.data);
      } else if (cacheData?.data?.status === 'success' && Array.isArray(cacheData.data.data)) {
        setTYLScores(cacheData.data.data);
      } else {
        setTYLScores([]);
      }
    }
  ...
  const skillsData = [
    { skill: 'Language proficiency in English', description: 'Language Proficience is described as below mentioned levels:\nA1-Beginner\nA2-Elementary\nB1-Intermediate\nB2-Upper Intermediate\nC1-Advanced\nC2-Proficient', score: '1 for A1\n 2 for A2\n 3 for B1\n 4 for B2\n 5 for C1/C2', required: '4', comments: 'For effective business communication in Corporate World' },
    { skill: 'Aptitude', description: 'Aptitude has 3 Components:\n1. Verbal Ability\n2.Quantitative\n3. Logical Reasoning ', score: 'Test Score of 1 to 5 mapped to absolute score', required: '3', comments: 'Will use one of Aptitude Assessment(TBD)' },
    { skill: 'Core Technical', description: 'Fundamentals of Core subjects', score: 'Results rating on scale of 1 to 5', required: '3', comments: 'Administer a test similar to GATE' },
    { skill: 'Core Technical', description: 'Certifications in identified technical courses like: Python, DB, MATLAB, R Programming', score: 'No. of Cert.(with to 50%)', required: '4', comments: '' },
    { skill: 'Experiential', description: 'Mini Projects', score: 'No. of Projects', required: '4', comments: '' },
    { skill: 'Experiential', description: 'Internship', score: 'No. of Weeks/2', required: '4', comments: '' },
    { skill: 'Soft Skills', description: 'Presentation skills, Group Discussion, Business Etiquette', score: 'Trainer Rating on scale of 1 to 5', required: '3', comments: 'Will use ratings from Trainer' }
  ];

  const tableCellStyle = { border: `1px solid ${theme.palette.divider}`, backgroundColor: theme.palette.background.paper };
                ...
                <Table sx={{ minWidth: { xs: 560, md: '100%' } }}>
                  ...
                  <TableBody>
                    {parameters.map((parameter) => (
                      <TableRow key={parameter} sx={{ '&:nth-of-type(odd)': { backgroundColor: theme.palette.mode === 'light' ? theme.palette.grey[50] : theme.palette.grey[900] } }}>
                        <TableCell sx={tableCellStyle}>{parameter}</TableCell>
                        <TableCell sx={tableCellStyle}>
                          <TextField fullWidth size="small" color={colorMode} name={`${parameter}-target`} disabled={!isFaculty} value={getCurrentSemesterScores(parameter, 'target')} onChange={(e) => handleScoreChange(parameter, selectedSemester, 'target', e.target.value)} />
                        </TableCell>
                        <TableCell sx={tableCellStyle}>
                          <TextField fullWidth size="small" color={colorMode} name={`${parameter}-actual`} disabled={!isFaculty} value={getCurrentSemesterScores(parameter, 'actual')} onChange={(e) => handleScoreChange(parameter, selectedSemester, 'actual', e.target.value)} />
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
              ...
              <Table sx={{ minWidth: { xs: 980, md: '100%' } }}>
                ...
                <TableBody>
                  {skillsData.map((row, index) => (
                    <TableRow key={index} sx={{ '&:nth-of-type(odd)': { backgroundColor: theme.palette.mode === 'light' ? theme.palette.grey[50] : theme.palette.grey[900] } }}>
